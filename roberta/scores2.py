...
def main():
    ...
    scores={key: loss_gap_after[key] - loss_gap_before[key] for key in loss_gap_after if
                       key in loss_gap_before}

    score = [scores[key] for key in scores]
    score_file = f"{training_args.output_dir}/scores_{training_args.task_name}_{config.target_ratio}_{training_args.weight_decay}_{training_args.seed}_{training_args.num_train_epochs}_{training_args.state}_{config.reg}_{config.pruneFlag}.csv"
...
if __name__ == "__main__":
    ...
    # python roberta/scores2.py --state ft --dataset mrpc --seed 3404 --reg 5e-8 --weight_decay 0.0 --epoch 5 --remain_loss 1 --model bert-base-uncased --target_ratio 0.5 --pruneFlag down
