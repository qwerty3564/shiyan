...
def main():
    ...
    scores1={key: loss_gap_after[key] for key in loss_gap_after if key in loss_gap_before}
    scores2={key: abs(loss_gap_after[key]-loss_gap_before[key])/loss_gap_before[key] for key in loss_gap_after if key in loss_gap_before}
    sorted_scores1 = torch.tensor([scores1[key] for key in sorted(scores1.keys())])
    sorted_scores2 = torch.tensor([scores2[key] for key in sorted(scores2.keys())])
    print(len(sorted_scores1))
    print(len(sorted_scores2))
    keep_ratio = 1.1-config.target_ratio
    threshold, _ = torch.topk(sorted_scores1, int(keep_ratio * len(sorted_scores1)))
    idx = sorted_scores1 < threshold[-1]
    sorted_scores2[idx] = 0
    ...
    data_p.prune()
    index = torch.arange(0,len(sorted_scores2))
    data_p.update(sorted_scores2, index)
