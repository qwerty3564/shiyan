...
def main():
    ...
    if config.pruneFlag=="random":
        ...
        loss_gap1 = {key: torch.sum(torch.abs(loss_after[key] - loss_before[key])).item() for key in loss_after if key in loss_before}
        #欧氏距离
    ...
    if config.pruneFlag=="random":
        common_keys = [key for key in loss_after if key in loss_before]
        loss_gap2 = {key: index for index, key in enumerate(common_keys)}
    else:
        # loss_gap = {key: torch.max(torch.abs(loss_after[key] - loss_before[key])).item() for key in loss_after.keys() if key in loss_before.keys()}
        loss_gap2 = {key: torch.sum(torch.abs(loss_after[key] - loss_before[key])).item() for key in loss_after if key in loss_before}
        #欧氏距离
        # loss_gap = {key: torch.sqrt(torch.sum(torch.square(loss_after[key] - loss_before[key]))).item() for key in loss_after if key in loss_before}
        # loss_gap = {key: torch.var(loss_after[key] - loss_before[key]).item() for key in loss_after if key in loss_before}
    score2 = loss_gap2
    ...
    for i in range(len(loss_after)):
        pooler_gap = [(a - b).item() for a, b in zip(loss_after[i], loss_before[i])]
        pooler_gap_file = f"pooler_gap_{i+1}_{config.dataset}_{config.reg}_{config.seed}.csv"
        df = pd.DataFrame(pooler_gap)
        df.to_csv(pooler_gap_file, index=False)
        # pooler_gap = [a.item() for a in loss_before[i]]
        # pooler_gap_file = f"pooler_gap0_{i + 1}_{config.dataset}_{config.reg}_{config.seed}.csv"
        # df = pd.DataFrame(pooler_gap)
        # df.to_csv(pooler_gap_file, index=False)
        # pooler_gap = [a.item() for a in loss_after[i]]
        # pooler_gap_file = f"pooler_gap1_{i + 1}_{config.dataset}_{config.reg}_{config.seed}.csv"
        # df = pd.DataFrame(pooler_gap)
        # df.to_csv(pooler_gap_file, index=False)
    scores1 = [score1[key] for key in score1]
    scores_file1 = f"scores1_{modelname}_{config.dataset}_{config.seed}_{config.reg}_{config.batchsize}.csv"
    df = pd.DataFrame(scores1)
    df.to_csv(scores_file1, index=False)
    scores2 = [score2[key] for key in score2]
    scores_file2 = f"scores2_{modelname}_{config.dataset}_{config.seed}_{config.reg}_{config.batchsize}.csv"
    df = pd.DataFrame(scores2)
    df.to_csv(scores_file2, index=False)
    s=[]
    small=0
    big=0
    total=0
    for key in score1:
        if score2[key] != 0:
            s.append(score1[key]/score2[key])
            if score1[key]/score2[key] >1:
                big+=1
            elif score1[key]/score2[key] <1:
                small +=1
        else:
            s.append(1)
        total+=1
    s = f"compress big: {big/total:.5f}"
    log.info(s)
    s = f"expand big: {small/total:.5f}"
    log.info(s)
    end_time = time.time()
...
if __name__ == "__main__":
    ...
    #细微压缩和扩大
    #保存s1，以及32个样本的768维颗粒
    # python roberta/s1_2.py --state ft --dataset mrpc --seed 3404 --reg 5e-8 --weight_decay 0.0 --epoch 1 --remain_loss 1 --model bert-base-uncased
