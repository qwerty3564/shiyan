...
def main():
    ...
    if config.pruneFlag == "random":
        ...
        loss_gap1 = {key: torch.sum(torch.abs(loss_after[key] - loss_before[key])).item() for key in loss_after if
                     key in loss_before}
        # 欧氏距离
    ...
    if config.pruneFlag == "random":
        ...
        # loss_gap = {key: torch.max(torch.abs(loss_after[key] - loss_before[key])).item() for key in loss_after.keys() if key in loss_before.keys()}
        loss_gap2 = {key: torch.sum(torch.abs(loss_after[key] - loss_before[key])).item() for key in loss_after if
                     key in loss_before}
        # 欧氏距离
        # loss_gap = {key: torch.sqrt(torch.sum(torch.square(loss_after[key] - loss_before[key]))).item() for key in loss_after if key in loss_before}
        # loss_gap = {key: torch.var(loss_after[key] - loss_before[key]).item() for key in loss_after if key in loss_before}
    score2 = loss_gap2
    scores={key: abs(loss_gap1[key]-loss_gap2[key])*loss_gap1[key]/(loss_gap2[key]+0.0000001) for key in loss_gap2 if
                       key in loss_gap1}
    ...
    # 输出结果
    print(f"compress big: {positive_ratio:.5f}")
    print(f"expand big: {negative_ratio:.5f}")
    s=f"compress big: {positive_ratio:.5f}"
    log.info(s)
    s = f"expand big: {negative_ratio:.5f}"
    log.info(s)
...
if __name__ == "__main__":
    ...
    # 测试集样本在微调前后的颗粒状况
    #先统计每个样本微调前后的散度（得分)
    # python ../../traindata3_2_1.py --state ft --dataset mrpc --seed 3404 --reg 5e-8 --weight_decay 0.0 --epoch0 1 --epoch 10 --remain_loss 1 --model bert-base-uncased --target_ratio 0.5 --pruneFlag up --optim adamw_torch --learning_rate 2e-5