...
def main():
    ...
    mean=sum(loss_gap_after.values())/len(loss_gap_after)
    scores={key: abs(loss_gap_after[key]-loss_gap_before[key])+abs(loss_gap_after[key]-mean)+abs(loss_gap_before[key]-mean) for key in loss_gap_after if
...
if __name__ == "__main__":
    ...
    # 剪枝标准：
        # mean=mean(s2)
        # abs(s2-s1)+abs(s2-mean)+abs(s1-mean)
    # 测试集样本在微调前后的颗粒状况
    #先统计每个样本微调前后的散度（得分)
    # python ../../traindata3_5.py --state ft --dataset mrpc --seed 3404 --reg 5e-8 --weight_decay 0.0 --epoch0 1 --epoch 10 --remain_loss 1 --model bert-base-uncased --target_ratio 0.5 --pruneFlag up --optim adamw_torch --learning_rate 2e-5