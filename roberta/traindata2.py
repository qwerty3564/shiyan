...
def main():
    ...
    scores={key: abs(loss_gap_after[key] - loss_gap_before[key]) for key in loss_gap_after if
                       key in loss_gap_before}
    import shutil
    ...
    # 创建DataFrame
    df_weights = pd.DataFrame(weights, columns=[f'Feature_{i}' for i in range(weights.shape[1])])
    df_biases = pd.DataFrame(biases, columns=['Bias'])

    # 合并权重和偏置
    df = pd.concat([df_weights, df_biases], axis=1)

    # 保存到CSV文件
    df.to_csv('classifier_params0.csv', index=False)

    print("分类头的参数已保存到 classifier_params0.csv")
    # 创建Trainer实例
    trainer = GlueTrainer(
        model=model,
        args=training_args,
    ...
    # 保存到CSV文件
    df.to_csv('classifier_params1.csv', index=False)

    print("分类头的参数已保存到 classifier_params1.csv")
    s=f'{trainer.evaluate(eval_dataset)}'
    print(s)
    log.info(f'\n{s}\n')
...
if __name__ == "__main__":
    main()
    # 剪枝标准：
        # 初始模型微调前后对样本的768维颗粒的1Norm之差的绝对值
    # 测试集样本在微调前后的颗粒状况
    #先统计每个样本微调前后的散度（得分），再以两者之差作为数据选择的依据
    # python ../../traindata2.py --state ft --dataset mrpc --seed 3404 --reg 5e-8 --weight_decay 0.0 --epoch0 1 --epoch 10 --remain_loss 1 --model bert-base-uncased --target_ratio 0.5 --pruneFlag up --optim adamw_torch